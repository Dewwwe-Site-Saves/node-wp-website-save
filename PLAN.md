...
- `prisma` and `tsx` moved to `dependencies`: the runner carries the full production `node_modules` (`npm prune --omit=dev` at the end of the build stage, copied over the standalone output; a separate `prod-deps` stage was dropped because BuildKit ran it in parallel with the build and doubled the peak memory) so that `prisma migrate deploy` and the operator scripts run inside the container. `import-config.ts` runs through `docker exec`, no local preparation of the database.
...
- First `next build` ever on v2 (the phase 4 checks ran in `next dev` only) found two more things. `ssh2` loads a native binding that Turbopack cannot bundle: `ssh2` and `ssh2-sftp-client` joined `serverExternalPackages`. And the build-time tracer turned `path.join(process.cwd(), 'data')` into a reference to the whole data directory and every `path.join(x, 'name')` into a `**/name` glob over the project: with the clones inside the repo it enumerated 100k files, took 10 GB of RAM, froze an 8 GB Mac twice and copied the dumps into `.next/standalone`. Fixed with `turbopackIgnore` comments on the path helpers and the `.git` join (they do work on `path.join` in 16.3.4, despite [next.js#95125](https://github.com/vercel/next.js/issues/95125)) and `outputFileTracingExcludes` for `data/` as a safety net. Verified with the clones in the tree: 7.7 s compile, 950 MB peak, 64 MB standalone.
