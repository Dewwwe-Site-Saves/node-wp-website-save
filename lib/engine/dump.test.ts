...
function stubEndpoint(handler: (request: Request) => Response | Promise<Response>) {
    const fn = vi.fn(async (input: string | URL | Request, init?: RequestInit) => handler(new Request(input, init)));
...
describe('dumpDatabase', () => {
    it('uploads token hash and script, triggers, downloads, validates and cleans up', async () => {
        const fetchMock = stubEndpoint(request => {
            ...
            expect(remote.files.get('/www/dewwwe-backup.php')?.content.toString()).toContain('<?php');
    ...
    it('reports the endpoint error code and still removes script and token', async () => {
        stubEndpoint(() => json(500, { status: 'error', code: 'exec_disabled', message: 'exec() is disabled on this host' }));
        await expect(dumpDatabase(site, remote.client(), localPath, log)).rejects.toThrow('Dump failed (HTTP 500): exec_disabled: exec() is disabled on this host');
        expect(remote.has('/www/dewwwe-backup.php')).toBe(false);
        expect(remote.has('/www/.dewwwe-backup-token')).toBe(false);
    });
    ...
    it('truncates a non-JSON body', async () => {
        ...
        await expect(dumpDatabase(site, remote.client(), localPath, log)).rejects.toSatisfy((error: Error) => error.message.length < 700);
    });

    it('refuses to follow a redirect', async () => {
        stubEndpoint(() => new Response(null, { status: 301, headers: { Location: 'https://www.site.test/' } }));
        await expect(dumpDatabase(site, remote.client(), localPath, log)).rejects.toThrow('redirected to https://www.site.test/');
    });
    ...
    it('rejects a dump that is not SQL and removes it from the remote', async () => {
        ...
        await expect(dumpDatabase(site, remote.client(), localPath, log)).rejects.toThrow('does not look like SQL');
        expect(remote.has('/www/db_wp_abcd1234.sql')).toBe(false);
    });
    ...
    it('rejects a too small dump', async () => {
        ...
        await expect(dumpDatabase(site, remote.client(), localPath, log)).rejects.toThrow('too small');
    });

    it('rejects an unexpected remote file name', async () => {
        stubEndpoint(() => json(200, { status: 'ok', file: '../wp-config.php' }));
        await expect(dumpDatabase(site, remote.client(), localPath, log)).rejects.toThrow('unexpected file name');
    });
    ...
    it('turns an abort during the request into a cancellation', async () => {
        ...
        await expect(dumpDatabase(site, remote.client(), localPath, log, controller.signal)).rejects.toBeInstanceOf(BackupCancelledError);
        expect(remote.has('/www/dewwwe-backup.php')).toBe(false);
    });
});
