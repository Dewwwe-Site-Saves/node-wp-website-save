 ...
 */
export async function dumpDatabase(site: SiteConfig, client: RemoteClient, localPath: string, log: Logger, signal?: AbortSignal): Promise<DumpResult> {
    ...
    try {
        ...
        for (const remotePath of [remoteDump, remoteRootFile(site, DUMP_SCRIPT_NAME), remoteRootFile(site, TOKEN_FILE_NAME)]) {
...
async function triggerDump(domain: string, token: string, signal?: AbortSignal): Promise<string> {
    ...
    try {
        ...
        if (error instanceof Error && error.name === 'TimeoutError') throw new DumpError(`Dump request timed out after ${HTTP_TIMEOUT_MS / 60_000} min`);
        throw new DumpError(`Dump request failed: ${errorMessage(error)}`);
    }

    if (response.status >= 300 && response.status < 400) {
        throw new DumpError(`https://${domain} redirected to ${response.headers.get('location') ?? 'another URL'}; set the site domain to its canonical host`);
    }
    ...
    try {
        ...
        throw new DumpError(`Dump endpoint answered HTTP ${response.status} with a non-JSON body: ${text.slice(0, RESPONSE_SNIPPET)}`);
    }
    ...
    if (!REMOTE_DUMP_NAME.test(payload.file) || !isSafePath(payload.file)) {
        throw new DumpError(`Dump endpoint returned an unexpected file name: ${payload.file.slice(0, 100)}`);
    }
    return payload.file;
}
