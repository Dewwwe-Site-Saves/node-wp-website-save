...
//         if (config.sharepoint && config.tenantID) {
//             this.buffer = readFileSync("./lib/temp/key.pem");
...
//     async updateList(itemID) {
//         this.list = sp.web.lists.getByTitle(this.listName);

//         i = await list.items.getById(itemID).update({
//             [this.dateFieldName]: "My New Title"
//           });
...
// export default Sp;
    ...
    const updates = {
        ...
        [dateColumn]: date.toISOString()
      };
    console.log(updates);

    const i = await list.items.getById(1).update(updates);
