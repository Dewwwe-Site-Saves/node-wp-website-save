...
class Cleanup {
    ...
    cleanupSiteFolder() {
        ...
        if (fs.existsSync(this.localSitePath)) {
            ...
            try {
                ...
                this.checkOrCreate(this.tempPath+'.github/');
                this.checkOrCreate(this.tempPath+'.github/workflows/');
                this.copyFile(this.filesFolder + '/helpers/auto-tagged-release.yml', this.tempPath + '.github/workflows/auto-tagged-release.yml');
            }
    ...
    ensureGitFiles() {
        ...
        if (!fs.existsSync(this.localSitePath + '.gitignore')) {
            console.log('No .gitignore found. Adding.');
            this.copyFile(this.filesFolder + '/helpers/.gitignore', this.localSitePath + '.gitignore');
            mustCommitGitignore = true;
        }

        // Ensure .github/workflows exists
        this.checkOrCreate(this.localSitePath + '.github/');
        this.checkOrCreate(this.localSitePath + '.github/workflows/');
        try {
            this.copyFile(this.filesFolder + '/helpers/auto-tagged-release.yml', this.localSitePath + '.github/workflows/auto-tagged-release.yml');
        } catch {
            // Already exists (COPYFILE_EXCL), that's fine
        }
