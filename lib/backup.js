 ...
 */
function createLogger(prefix, quiet = false) {
    const lines = [];

    function write(level, ...args) {
        const msg = args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' ');
        const line = `${prefix} ${msg}`;
        lines.push({ level, msg: line, time: new Date().toISOString() });
        if (!quiet) {
...
export async function backupSite(domain, config, options = {}) {
    const { basePath, fullDownload = false, quiet = false } = options;
    const log = createLogger(`[${domain}]`, quiet);
    ...
    try {
        ...
        // Generate and upload backup token
        const backupToken = crypto.randomBytes(32).toString('hex');
        const tokenFilePath = basePath + '/helpers/.dewwwe-backup-token';
