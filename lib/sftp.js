    ...
    async downloadChanged() {
        ...
        try {
            ...
            for (let i = 0; i < toDownload.length; i += BATCH_SIZE) {
                ...
                await Promise.all(batch.map(f => sftp.fastGet(f.remotePath, f.localFilePath)));
            ...
            self.log.log(`Sync complete: ${toDownload.length} downloaded, ${skipped} unchanged, ${deleted} deleted`);

            return { downloaded: toDownload.length, unchanged: skipped, deleted };
        } catch (err) {
            throw err;
        } finally {
            sftp.end();
