...
export function validationError(error: ZodError): NextResponse {
    ...
    return jsonError(400, first ? `${first.path || 'input'}: ${first.message}` : 'Invalid input', { issues });
}

/** Parses and validates a JSON body. Returns either the data or the error response to send. */
export async function parseBody<T>(request: Request, schema: ZodType<T>): Promise<{ data: T; response?: never } | { data?: never; response: NextResponse }> {
    let raw: unknown;
    try {
