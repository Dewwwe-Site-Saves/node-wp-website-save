    ...
    async downloadChanged() {
        ...
        if (toDownload.length > 0) {
            ...
            for (let i = 0; i < toDownload.length; i++) {
                ...
                if ((i + 1) % 100 === 0) self.log.log(`  ${i + 1}/${toDownload.length} downloaded...`);
            }
            self.disconnect();
        }

        const deleted = deleteOrphans(self.localPath, remoteFiles);
        self.log.log(`Sync complete: ${toDownload.length} downloaded, ${scanned - toDownload.length} unchanged, ${deleted} deleted`);
    }
