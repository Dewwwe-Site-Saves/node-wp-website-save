...
export function getDb() {
    ...
    _db.exec(`
        CREATE TABLE IF NOT EXISTS sites (
            ...
            password TEXT,
            web_root_path TEXT NOT NULL DEFAULT 'www',
            ssh_key_path TEXT,
        ...
        CREATE TABLE IF NOT EXISTS backups (
            ...
            commit_sha TEXT,
            error_message TEXT,
            hack_alert INTEGER NOT NULL DEFAULT 0,
            hack_details TEXT,
            log TEXT
        );
...
export function updateSite(id, site) {
    return getDb().prepare(`
        ...
        sp_list_item_id=@sp_list_item_id, cron_schedule=@cron_schedule, enabled=@enabled, updated_at=CURRENT_TIMESTAMP
        WHERE id = @id
    `).run({ ...site, id });
}
...
export function getBackups({ siteId, status, limit = 50, offset = 0 } = {}) {
    let query = 'SELECT b.*, s.domain FROM backups b JOIN sites s ON b.site_id = s.id WHERE 1=1';
    const params = {};
...
export function updateBackup(id, data) {
    const allowed = ['finished_at', 'status', 'duration_ms', 'files_downloaded', 'files_unchanged',
        'files_deleted', 'dump_size_bytes', 'commit_sha', 'error_message', 'hack_alert', 'hack_details', 'log'];
    const fields = Object.keys(data).filter(k => allowed.includes(k));
    if (fields.length === 0) return;
    const setClause = fields.map(k => `${k} = @${k}`).join(', ');
    return getDb().prepare(`UPDATE backups SET ${setClause} WHERE id = @id`).run({ ...data, id });
}
