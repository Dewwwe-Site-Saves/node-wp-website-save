...
export function BackupHistory({ backups, showDomain = true, siteId }: { backups: BackupWithDomain[]; showDomain?: boolean; siteId?: number }) {
    ...
    function handleRowClick(backup: BackupWithDomain) {
        const runningJob = backup.status === 'running' ? runningJobs.find(j => j.siteId === backup.siteId) : undefined;
        if (runningJob) {
            setModal({ mode: 'live', jobId: runningJob.id, domain: runningJob.domain, siteId: backup.siteId });
        } else {
    ...
    const runningSiteIds = new Set(backups.filter(b => b.status === 'running').map(b => b.siteId));
    ...
    return (
        <>
                ...
                <TableBody>
                    {extraRunning.map(job => (
                        <TableRow key={`running-${job.id}`} className="cursor-pointer hover:bg-muted/50"
                            onClick={() => setModal({ mode: 'live', jobId: job.id, domain: job.domain, siteId: job.siteId })}>
                    ...
                    {backups.map(backup => (
                        <TableRow key={backup.id} className="cursor-pointer hover:bg-muted/50"
                            ...
                            {showDomain && <TableCell className="font-medium">{backup.site.domain}</TableCell>}
                            <TableCell>
                            ...
                            <TableCell className="hidden lg:table-cell">
                                <OptionsCell fullDownload={backup.fullDownload} skipGit={backup.skipGit} />
                            </TableCell>
                            <TableCell className="hidden sm:table-cell text-muted-foreground">
            ...
            {modal?.mode === 'history' && (
                <LogModal mode="history" backupId={modal.backup.id} domain={modal.backup.site.domain} siteId={modal.backup.siteId}
                    status={modal.backup.status}
                    startedAt={modal.backup.startedAt ? new Date(modal.backup.startedAt).toISOString() : undefined}
                    durationMs={modal.backup.durationMs} filesDownloaded={modal.backup.filesDownloaded}
                    filesUnchanged={modal.backup.filesUnchanged} dumpSizeBytes={modal.backup.dumpSizeBytes}
                    commitSha={modal.backup.commitSha}
                    fullDownload={modal.backup.fullDownload} skipGit={modal.backup.skipGit}
